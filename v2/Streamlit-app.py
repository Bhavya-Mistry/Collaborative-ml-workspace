...
st.markdown(
    ...
    .main-title { font-size: 2.6em; font-weight: 700; color: #1a237e; text-align: center; margin-top: 10px; }
    .section-header { font-size: 1.3em; color: #3949ab; font-weight: 600; margin-top: 1.2em; margin-bottom: 0.5em; }
    .stButton>button { background: linear-gradient(90deg, #3949ab 0%, #00c6ff 100%); color: white; border-radius: 8px; }
    </style>
...
def run_career_predictor():
    ...
    with main_col:
        ...
        cgpa = st.slider("Current CGPA", 0.0, 10.0, 7.5, 0.1, help="Your latest CGPA")
    ...
    if st.button("🔍 Predict My Career", use_container_width=True):
        ...
        with st.spinner("Predicting your career..."):
            try:
                ...
                st.error(f"Error: Could not connect to the backend. Please try again later. ({e})")

# ATS Resume Evaluator Logic
        ...
        with st.spinner("Evaluating ATS score..."):
            ...
            fig = go.Figure(go.Indicator(
                ...
                gauge={
                    ...
                    'steps': [{'range': [0, 5], 'color': '#ef5350'}, {'range': [5, 8], 'color': '#ffa726'}, {'range': [8, 10], 'color': '#26a69a'}],
                }
            ))
